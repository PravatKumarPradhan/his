		...
		[ '$scope','$rootScope','$state','$cookies','GenericService','PagerService','promiseFactory','$sessionStorage',
			'growl',
				function($scope,$rootScope,$state, $cookies, GenericService,PagerService,promiseFactory,$sessionStorage,growl)
				{
					...
					$scope.LISDynamicLabel = "Antibiotic Class";
					...
					$(".selectedPageName").text("Antibiotic Class Master");
					...
					$scope.initAntibioticsMaster = function(){
					$scope.antibioticsMaster =
					{
						 "antibioticClassId":$scope.common.id,
					...
					$scope.setNoOfRecords = function() {
						$scope.initAntibioticsMasterList($scope.orgId, $scope.offset, $scope.noOfRecordsPerPage);
					};
					$scope.initAntibioticsMasterList = function(orgId, offset, noOfRecordsPerPage)
					{
						var data = "";
						$rootScope.startLoader();
						offset = offset != null ? offset : 0;
						noOfRecordsPerPage = noOfRecordsPerPage != null ? noOfRecordsPerPage : 10;
						var URI1 = BASE_URL + ROOT_URL + LIS_GLOBAL + LIST_ANTIBIOTIC_MASTER + S + orgId + S + offset
								+ S + noOfRecordsPerPage;
						var URI2 = BASE_URL + ROOT_URL + LIS_GLOBAL + COUNT_ANTIBIOTIC_MASTER + S + orgId;
					...
					$scope.getAntibioticsMasterList = function(orgId, offset, noOfRecordsPerPage) {
						console.log("offset", offset);
						$rootScope.startLoader();
						var data = "";
						offset = offset != null ? offset : 0;
						noOfRecordsPerPage = noOfRecordsPerPage != null ? noOfRecordsPerPage : 10;
						var URI = BASE_URL + ROOT_URL + LIS_GLOBAL + LIST_ANTIBIOTIC_MASTER + S + orgId + S + offset
								+ S + noOfRecordsPerPage;
						console.log("URI", URI);
					...
					$scope.setPage = function(page, flag) {
						...
						if (flag)
						{
							$scope.getAntibioticsMasterList($scope.orgId, $scope.pager.startIndex, $scope.pager.pageSize);
						}
					}
					...
					$scope.updateStatus = function(commonId,status)
					{
						try {
							$rootScope.startLoader();
							$scope.initAntibioticsMaster();
							var URI = BASE_URL + ROOT_URL + LIS_GLOBAL+ACTIVE_INACTIVE_ANTIBIOTIC_MASTER+S+$scope.orgId+S+commonId+S+status;
							console.log("ACT_INACT_URI",URI);
							GenericService.serviceAction("GET", URI, $scope.antibioticsMaster).then(function(response) 
							{
					...
					$scope.showUpdateBtn = function(commonId)
					{
						try {
							...
							var URI = BASE_URL + ROOT_URL + LIS_GLOBAL+EDIT_ANTIBIOTIC_MASTER+S+$scope.orgId+S+commonId;
					...
					$scope.save = function() 
					{
						...
						if ($scope.commonform.$valid)
						{
							try {
								$rootScope.startLoader();
								$scope.initAntibioticsMaster();
								console.log("AntibioticsMaster",JSON.stringify($scope.antibioticsMaster));
								var URI = BASE_URL + ROOT_URL + LIS_GLOBAL+ADD_ANTIBIOTIC_MASTER;
								GenericService.serviceAction("POST", URI, $scope.antibioticsMaster).then(function(response) 
								{	
									if (response.data.status == 'success')
									{	
										$rootScope.stopLoader();
					...
					$scope.update = function() 
					{
						...
						if ($scope.commonform.$valid)
						{
							try {
								...
								$scope.initAntibioticsMaster();
								console.log("AntibioticsMaster",JSON.stringify($scope.antibioticsMaster));
								var URI = BASE_URL + ROOT_URL + LIS_GLOBAL+UPDATE_ANTIBIOTIC_MASTER;
								GenericService.serviceAction("PUT", URI, $scope.antibioticsMaster).then(function(response) 
								{
									if (response.data.status == 'success')
									{
										$rootScope.stopLoader();
