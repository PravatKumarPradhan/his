...
angular
		.module('myApp')
		.controller(
				"deatilsReportEntryWorklistController",
				[
						'$scope',
						'$rootScope',
						'$state',
						...
						function($scope, $rootScope, $state, $cookies,
							...
							$scope.getBiochemResultEntryDetails = function() 
							{
								...
								try {
									...
									console.log("$scope.patientId Details",$scope.patientId);
									...
									var URI = BASE_URL + ROOT_URL
											+ LIS_BIOCHEMISTRY
											+ GET_WORK_LIST_DEPT_RESULT_ENTERY_DETAILS;
									console.log("URI", URI);
							...
							$scope.createLabResultObjectList = function()
							{
								angular.forEach($scope.testMasterResList, function(value, key) {
									...
									if(value.testType==0)
									{
									      ...
									      $scope.labResultMasterDto.resultLevel = RESULT_ENTRY;
							...
							$scope.saveResultEntryDetails= function() 
							{
								...
								if ($scope.worklistForReportEntryForm.$valid)
								{
									...
									{
										$rootScope.startLoader();
										$scope.labResultMasterDto.sampleStatusId = RESULT_ENTRY_DONE;
										$scope.createLabResultObjectList();
										console.log("labResultMasterDtoList",JSON.stringify($scope.labResultMasterDtoList));
										var URI =BASE_URL + ROOT_URL
										+ LIS_BIOCHEMISTRY
										+ WORK_RESULT_ENTERY_DETAILS;
										console.log("URI",URI);
										...
										{	
											if (response.data.status == 'success')
											{
												...
												$state.go('worklistForReportEntry');
							...
							$scope.getRetestRecollectTests = function()
							{
								  try {
									   $scope.previousTest = [];
										...
										console.log("PREVIOUS TEST URI", URI);
										console.log("PREVIOUS",JSON.stringify(data));
										return GenericService
												.serviceAction(METHOD_POST, URI, data)
												.then(
														function(response) {
															$rootScope.stopLoader();
															console.log("$scope.previousTest",JSON.stringify(response.data.object));
															return $scope.previousTest  = response.data.object;
							  ...
							  $scope.datasetOverride = [{
								...
								{  
									...
									try {
										...
										console.log("PARAMETER HISTORY URI", URI);
										console.log("PARAMETER HISTORY DATA", JSON.stringify(data));
