		...
		[ '$scope','$rootScope','$state','$cookies','GenericService','PagerService','promiseFactory','$sessionStorage',
			'growl',
				function($scope,$rootScope,$state, $cookies, GenericService,PagerService,promiseFactory,$sessionStorage,growl)
				{
					...
					$(".selectedPageName").text("Media Master");
					...
					$scope.initMediaMaster = function(){
					$scope.mediaMaster =
					{
						 "mediaId":$scope.common.id,
					...
					// ====================================================CODE START FORFOR MEDIA MASTER LIST===========================================================

					$scope.setNoOfRecords = function() {
						$scope.initMediaMasterList($scope.orgId, $scope.offset, $scope.noOfRecordsPerPage);
					};
					$scope.initMediaMasterList = function(orgId, offset, noOfRecordsPerPage)
					{
						var data = "";
						$rootScope.startLoader();
						offset = offset != null ? offset : 0;
						noOfRecordsPerPage = noOfRecordsPerPage != null ? noOfRecordsPerPage : 10;
						var URI1 = BASE_URL + ROOT_URL + LIS_GLOBAL + LIST_MEDIA_MASTER + S + orgId + S + offset
								+ S + noOfRecordsPerPage;
						var URI2 = BASE_URL + ROOT_URL + LIS_GLOBAL + COUNT_MEDIA_MASTER + S + orgId;
					...
					$scope.getMediaMasterList = function(orgId, offset, noOfRecordsPerPage) {
						console.log("offset", offset);
						var data = "";
						$rootScope.startLoader();
						offset = offset != null ? offset : 0;
						noOfRecordsPerPage = noOfRecordsPerPage != null ? noOfRecordsPerPage : 10;
						var URI = BASE_URL + ROOT_URL + LIS_GLOBAL + LIST_MEDIA_MASTER + S + orgId + S + offset
								+ S + noOfRecordsPerPage;
						console.log("URI", URI);
					...
					$scope.setPage = function(page, flag) {
						...
						if (flag)
						{
							$scope.getMediaMasterList($scope.orgId, $scope.pager.startIndex, $scope.pager.pageSize);
						}
					}

					$scope.initMediaMasterList($scope.orgId, $scope.offset, $scope.noOfRecordsPerPage);

					// ====================================================CODE END FOR MEDIA MASTER LIST===========================================================
					...
					$scope.updateStatus = function(commonId,status)
					{
						try {
							$rootScope.startLoader();
							$scope.initMediaMaster();
							var URI = BASE_URL + ROOT_URL + LIS_GLOBAL+ACTIVE_INACTIVE_MEDIA_MASTER+S+$scope.orgId+S+commonId+S+status;
							console.log("ACT_INACT_URI",URI);
							GenericService.serviceAction("GET", URI, $scope.mediaMaster).then(function(response) 
							{
					...
					$scope.showUpdateBtn = function(commonId)
					{
						try {
							...
							var URI = BASE_URL + ROOT_URL + LIS_GLOBAL+EDIT_MEDIA_MASTER+S+$scope.orgId+S+commonId;
					...
					$scope.save = function() 
					{
						...
						if ($scope.commonform.$valid)
						{
							try {
								$rootScope.startLoader();
								$scope.initMediaMaster();
								console.log("MediaMaster",JSON.stringify($scope.mediaMaster));
								var URI = BASE_URL + ROOT_URL + LIS_GLOBAL+ADD_MEDIA_MASTER;
								GenericService.serviceAction("POST", URI, $scope.mediaMaster).then(function(response) 
								{	
									if (response.data.status == 'success')
									{
										$rootScope.stopLoader();
					...
					$scope.update = function() 
					{
						...
						if ($scope.commonform.$valid)
						{
							try {
								...
								$scope.initMediaMaster();
								console.log("MediaMaster",JSON.stringify($scope.mediaMaster));
								var URI = BASE_URL + ROOT_URL + LIS_GLOBAL+UPDATE_MEDIA_MASTER;
								GenericService.serviceAction("PUT", URI, $scope.mediaMaster).then(function(response) 
								{
									if (response.data.status == 'success')
									{
										$rootScope.stopLoader();
