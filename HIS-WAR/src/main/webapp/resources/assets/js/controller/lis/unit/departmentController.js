		...
		[ '$scope','$rootScope','$state','$cookies','GenericService','PagerService','promiseFactory','$sessionStorage',
			'growl',
				function($scope,$rootScope,$state, $cookies, GenericService,PagerService,promiseFactory,$sessionStorage,growl)
				{
					...
					$scope.initDepartmentMaster = function(){
					$scope.departmentMaster =
					{
						 "departmentId":$scope.common.id,
					...
					// ====================================================CODE START FORFOR DEPARTMENT MASTER LIST===========================================================

					$scope.setNoOfRecords = function() {
						$scope.initDepartmentMasterList($scope.orgId, $scope.offset, $scope.noOfRecordsPerPage);
					};
					$scope.initDepartmentMasterList = function(orgId, offset, noOfRecordsPerPage)
					{
						var data = "";
						$rootScope.startLoader();
						offset = offset != null ? offset : 0;
						noOfRecordsPerPage = noOfRecordsPerPage != null ? noOfRecordsPerPage : 10;
						var URI1 = BASE_URL + ROOT_URL + LIS_GLOBAL + LIST_DEPARTMENT_MASTER + S + orgId + S + offset
								+ S + noOfRecordsPerPage;
						var URI2 = BASE_URL + ROOT_URL + LIS_GLOBAL + COUNT_DEPARTMENT_MASTER + S + orgId;
					...
					$scope.getDepartmentMasterList = function(orgId, offset, noOfRecordsPerPage) {
						console.log("offset", offset);
						var data = "";
						$rootScope.startLoader();
						offset = offset != null ? offset : 0;
						noOfRecordsPerPage = noOfRecordsPerPage != null ? noOfRecordsPerPage : 10;
						var URI = BASE_URL + ROOT_URL + LIS_GLOBAL + LIST_DEPARTMENT_MASTER + S + orgId + S + offset
								+ S + noOfRecordsPerPage;
						console.log("URI", URI);
					...
					$scope.setPage = function(page, flag) {
						...
						if (flag)
						{
							$scope.getDepartmentMasterList($scope.orgId, $scope.pager.startIndex, $scope.pager.pageSize);
						}
					}

					$scope.initDepartmentMasterList($scope.orgId, $scope.offset, $scope.noOfRecordsPerPage);

					// ====================================================CODE END FOR DEPARTMENT MASTER LIST===========================================================
					...
					$scope.updateStatus = function(commonId,status)
					{
						try {
							$rootScope.startLoader();
							$scope.initDepartmentMaster();
							var URI = BASE_URL + ROOT_URL + LIS_GLOBAL+ACTIVE_INACTIVE_DEPARTMENT_MASTER+S+$scope.orgId+S+commonId+S+status;
							console.log("ACT_INACT_URI",URI);
							GenericService.serviceAction("GET", URI, $scope.departmentMaster).then(function(response) 
							{
					...
					$scope.showUpdateBtn = function(commonId)
					{
						try {
							...
							var URI = BASE_URL + ROOT_URL + LIS_GLOBAL+EDIT_DEPARTMENT_MASTER+S+$scope.orgId+S+$scope.unitId+S+commonId;
					...
					$scope.save = function() 
					{
						...
						if ($scope.commonform.$valid)
						{
							try {
								$rootScope.startLoader();
								$scope.initDepartmentMaster();
								console.log("DepartmentMaster",JSON.stringify($scope.departmentMaster));
								var URI = BASE_URL + ROOT_URL + LIS_GLOBAL+ADD_DEPARTMENT_MASTER;
								GenericService.serviceAction("POST", URI, $scope.departmentMaster).then(function(response) 
								{	
									if (response.data.status == 'success')
									{
										$rootScope.stopLoader();
					...
					$scope.update = function() 
					{
						...
						if ($scope.commonform.$valid)
						{
							try {
								...
								$scope.initDepartmentMaster();
								console.log("DepartmentMaster",JSON.stringify($scope.departmentMaster));
								var URI = BASE_URL + ROOT_URL + LIS_GLOBAL+UPDATE_DEPARTMENT_MASTER;
								GenericService.serviceAction("PUT", URI, $scope.departmentMaster).then(function(response) 
								{
									if (response.data.status == 'success')
									{
										$rootScope.stopLoader();
