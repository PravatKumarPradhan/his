		...
		[ '$scope','$rootScope','$state','$cookies','GenericService','PagerService','promiseFactory','$sessionStorage',
			'growl',
				function($scope,$rootScope,$state, $cookies, GenericService,PagerService,promiseFactory,$sessionStorage,growl)
				{
					...
					$scope.initReagentMaster = function(){
					$scope.reagentMaster =
					{
						 "reagentId":$scope.common.id,
					...
					// ====================================================CODE START FORFOR REAGENT MASTER LIST===========================================================

					$scope.setNoOfRecords = function() {
						$scope.initReagentMasterList($scope.orgId, $scope.offset, $scope.noOfRecordsPerPage);
					};
					$scope.initReagentMasterList = function(orgId, offset, noOfRecordsPerPage)
					{
						$rootScope.startLoader();
						var data = "";
						offset = offset != null ? offset : 0;
						noOfRecordsPerPage = noOfRecordsPerPage != null ? noOfRecordsPerPage : 10;
						var URI1 = BASE_URL + ROOT_URL + LIS_GLOBAL + LIST_REAGENT_MASTER + S + orgId + S + offset
								+ S + noOfRecordsPerPage;
						var URI2 = BASE_URL + ROOT_URL + LIS_GLOBAL + COUNT_REAGENT_MASTER + S + orgId;
					...
					$scope.getReagentMasterList = function(orgId, offset, noOfRecordsPerPage) {
						console.log("offset", offset);
						var data = "";
						$rootScope.startLoader();
						offset = offset != null ? offset : 0;
						noOfRecordsPerPage = noOfRecordsPerPage != null ? noOfRecordsPerPage : 10;
						var URI = BASE_URL + ROOT_URL + LIS_GLOBAL + LIST_REAGENT_MASTER + S + orgId + S + offset
								+ S + noOfRecordsPerPage;
						console.log("URI", URI);
					...
					$scope.setPage = function(page, flag) {
						...
						if (flag)
						{
							$scope.getReagentMasterList($scope.orgId, $scope.pager.startIndex, $scope.pager.pageSize);
						}
					}

					$scope.initReagentMasterList($scope.orgId, $scope.offset, $scope.noOfRecordsPerPage);

					// ====================================================CODE END FOR REAGENT MASTER LIST===========================================================
					...
					$scope.updateStatus = function(commonId,status)
					{
						try {
							$rootScope.startLoader();
							$scope.initReagentMaster();
							var URI = BASE_URL + ROOT_URL + LIS_GLOBAL+ACTIVE_INACTIVE_REAGENT_MASTER+S+$scope.orgId+S+commonId+S+status;
							console.log("ACT_INACT_URI",URI);
							GenericService.serviceAction("GET", URI, $scope.reagentMaster).then(function(response) 
							{
					...
					$scope.showUpdateBtn = function(commonId)
					{
						try {
							...
							var URI = BASE_URL + ROOT_URL + LIS_GLOBAL+EDIT_REAGENT_MASTER+S+$scope.orgId+S+commonId;
					...
					$scope.save = function() 
					{
						...
						if ($scope.commonform.$valid)
						{
							try {
								$rootScope.startLoader();
								$scope.initReagentMaster();
								console.log("ReagentMaster",JSON.stringify($scope.reagentMaster));
								var URI = BASE_URL + ROOT_URL + LIS_GLOBAL+ADD_REAGENT_MASTER;
								GenericService.serviceAction("POST", URI, $scope.reagentMaster).then(function(response) 
								{	
									if (response.data.status == 'success')
									{
										$rootScope.stopLoader();
					...
					$scope.update = function() 
					{
						...
						if ($scope.commonform.$valid)
						{
							try {
								...
								$scope.initReagentMaster();
								console.log("ReagentMaster",JSON.stringify($scope.reagentMaster));
								var URI = BASE_URL + ROOT_URL + LIS_GLOBAL+UPDATE_REAGENT_MASTER;
								GenericService.serviceAction("PUT", URI, $scope.reagentMaster).then(function(response) 
								{
									if (response.data.status == 'success')
									{
										$rootScope.stopLoader();
