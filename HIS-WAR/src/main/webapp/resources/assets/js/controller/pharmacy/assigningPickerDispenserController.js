(function () {
    ...
    function assigningPickerDispenserController($scope, $rootScope, $http, CONSTANTS, GenericService, $filter, $state, growl, CurrencyService, assigningPickerDispenserService) {
        var self = this;
        ...
        function initializeController() {
            ...
            self.model.AssigningPickerDispenser = AssigningPickerDispenser;
            self.model.NavigateToDetail = NavigateToDetail;

            PopulateDropdown();
            AssigningPickerDispenser();
        }
        ...
        var AssigningPickerDispenser = function () {
            var URI = CONSTANTS.PHARMACY.OP.ASSIGN_PICKER_DISPENSER.SEARCH_API;
            ...
            GenericService.serviceAction("POST", URI, data).then(
                function (response) {
                    self.model.assigningPickerDispenser = response.data;
                })
            $rootScope.stopLoader();
        }

        var NavigateToDetail = function (item) {
            assigningPickerDispenserService.setAssigningPickerDispenserId(item.id);
            $state.go('assigningPickerDispenserDetails');
        }
        initializeController();
    }

    function config($stateProvider) {
        $stateProvider.state("assigningPickerDispenser", {
            url: "/assigningPickerDispenser",
            templateUrl: 'views/PharmacyInventory/InPatient/assigning-picker-dispenser.html',
            controller: "AssigningPickerDispenserController.Controller",
            controllerAs: "vm"
        });
    }
    ...
    angular
        .module("myApp")
        .config(config)
        .controller("AssigningPickerDispenserController.Controller", assigningPickerDispenserController)
})();
