...
angular
		.module('myApp')
		.controller(
				"detailsReportReleaseController",
				[
						'$scope',
						'$rootScope',
						'$state',
						...
						function($scope, $rootScope, $state, $cookies,
							...
							$scope.getBiochemResultEntryDetails = function() 
							{
								...
								try {
									...
									var URI = BASE_URL + ROOT_URL
											+ LIS_BIOCHEMISTRY
											+ GET_WORK_LIST_REPORT_RELEASE_DETAILS;
									console.log("URI", URI);
									GenericService
							...
							if(angular.equals({},$stateParams.resultMstObj))
 							{
								$state.go('worklistForReportRelease');
							} else {
							...
							$scope.createLabResultObjectList = function(flag)
							{
								angular.forEach($scope.testMasterResList, function(value, key) {
									...
									if(value.testType==0)
									{
									      ...
									      $scope.labResultMasterDto.resultLevel = RESULT_RELEASE;
							  ...
							  $scope.datasetOverride = [{
								...
								{  
									...
									try {
										...
										console.log("URI TABULAR", URI);
										console.log("data TABULAR", JSON.stringify(data));
										GenericService
												.serviceAction(METHOD_POST, URI, data)
												.then(
														function(response) {
