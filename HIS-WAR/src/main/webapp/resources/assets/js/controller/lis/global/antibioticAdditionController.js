		...
		[ '$scope','$rootScope','$state','$cookies','GenericService','PagerService','promiseFactory','$sessionStorage',
			'growl',
				function($scope,$rootScope,$state, $cookies, GenericService,PagerService,promiseFactory,$sessionStorage,growl)
				{
					...
					$scope.LISDynamicLabel = "Antibiotic Addition";
					...
					$(".selectedPageName").text("Antibiotic Addition Master");
					...
					{
						 "antiboiticMpprId":$scope.common.antiboiticMpprId,
					...
					$scope.getAntibioticClassList = function() {
						try {
							...
							var URI = BASE_URL + ROOT_URL
							        + LIS_GLOBAL
									+ GET_ANTIBIOTIC_ADDTION_ANTIBIOTIC_CLASS_LIST;
							console.log("URI", URI);
							GenericService
									.serviceAction("GET", URI, data)
									.then(
					...
					$scope.initAntibioticAdditionMasterList = function(orgId, offset, noOfRecordsPerPage)
					{
						...
						var URI1 = BASE_URL + ROOT_URL + LIS_GLOBAL + GET_TOTAL_ANTIBIOTIC_ADDTION_ANTIBIOTIC_CLASS_LIST + S + orgId + S + offset
								+ S + noOfRecordsPerPage;
						var URI2 = BASE_URL + ROOT_URL + LIS_GLOBAL + GET_COUNT_ANTIBIOTIC_ADDTION_ANTIBIOTIC_CLASS_LIST + S + orgId;
					...
					$scope.updateStatus = function(commonId,status)
					{
						try {
							...
							var URI = BASE_URL + ROOT_URL + LIS_GLOBAL+ACTIVE_INACTIVE_INCUBATION_ANTIBIOTIC_ADDTION_MASTER+S+$scope.orgId+S+commonId+S+status;
						...
						if ($scope.commonform.$valid)
						{
							try {
								...
								$scope.listAntibioticAdditionMaster =
								{
									 ...
									 "updatedBy":$scope.common.updatedBy,
									 "updatedDate":$scope.common.updatedDate,
								     "selectAntibioticClassList":[]
								     
								 }
								angular.forEach($scope.selectedAntibioticList,function(value,index){
										$scope.listAntibioticAdditionMaster.selectAntibioticClassList.push(value.antibioticId);
								})
								console.log("AntibioticAdditionMaster",JSON.stringify($scope.listAntibioticAdditionMaster));
								var URI = BASE_URL + ROOT_URL + LIS_GLOBAL+ADD_ANTIBIOTIC_ADDTION_MASTER;
