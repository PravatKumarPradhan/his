...
@NamedQueries({
	@NamedQuery(name = "GET_SENSITIVITY_TYPE_RESULT_BY_SENSITIVITY_ID", query = "SELECT sensitivityTestResultMaster.sensitivityId as id,"
			+ " sensitivityTestResultMaster.code as code,"
			+ " sensitivityTestResultMaster.desc as desc,"
			+ " sensitivityTestResultMaster.status as status,"
			...
			+ " WHERE sensitivityTestResultMaster.sensitivityId = :sensitivityId "
			+ " AND sensitivityTestResultMaster.orgId= :orgId"),
			
		@NamedQuery(name = "GET_SENSITIVITY_TYPE_RESULT_BY_SENSITIVITY_CODE", query = "SELECT sensitivityTestResultMaster.sensitivityId as id,"
			+ " sensitivityTestResultMaster.code as code,"
			+ " sensitivityTestResultMaster.desc as desc,"
			+ " sensitivityTestResultMaster.status as status,"
			+ " sensitivityTestResultMaster.createdBy as createdBy,"
			...
			+ " sensitivityTestResultMaster.updatedDate as updatedDate"
			+ " FROM SensitivityTestResultMaster sensitivityTestResultMaster"
			+ " WHERE sensitivityTestResultMaster.orgId=:orgId "
			+ " AND lower(sensitivityTestResultMaster.code) = lower(:code)"),
			
			@NamedQuery(name = "UPDATE_GET_SENSITIVITY_TYPE_RESULT_BY_SENSITIVITY_CODE", query = "SELECT sensitivityTestResultMaster.sensitivityId as id,"
					+ " sensitivityTestResultMaster.code as code,"
					+ " sensitivityTestResultMaster.desc as desc,"
					+ " sensitivityTestResultMaster.status as status,"
					+ " sensitivityTestResultMaster.createdBy as createdBy,"
					...
					+ " WHERE sensitivityTestResultMaster.orgId=:orgId "
					+ " AND lower(sensitivityTestResultMaster.code) = lower(:code)"
					+ " AND sensitivityTestResultMaster.sensitivityId <> :sensitivityId"),

	@NamedQuery(name = "GET_SENSITIVITY_TYPE_RESULT_MASTER_LIST", query = "SELECT sensitivityTestResultMaster.sensitivityId as id,"
			+ " sensitivityTestResultMaster.code as code,"
			+ " sensitivityTestResultMaster.desc as desc,"
			+ " sensitivityTestResultMaster.status as status,"
			+ " sensitivityTestResultMaster.createdBy as createdBy,"
